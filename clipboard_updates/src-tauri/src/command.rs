...
pub async fn google_login(
    ...
    // 1) Start loopback HTTP server on free port
    let server = Server::http("127.0.0.1:0").map_err(|e| {
        let msg = format!("Failed to start local OAuth server: {}", e);
        eprintln!("❌ {}", msg);
        msg
    })?;
   let port = match server.server_addr() {
    ...
    other => {
        eprintln!("❌ Unexpected listen addr: {:?}", other);
        return Err("Failed to determine local server port".to_string());
    }
};

    let redirect_uri = format!("http://127.0.0.1:{}/callback", port);
    println!("🌐 Using redirect URI: {}", redirect_uri);

    // 2) Init GoogleOAuth with this redirect_uri
    let mut cfg = GoogleOAuthConfig::default();
    cfg.redirect_uri = redirect_uri.clone();
    let oauth = GoogleOAuth::new(cfg);

    // 3) Generate auth URL (PKCE + state)
    let (auth_url, _state) = oauth.generate_auth_url()?;
    println!("🌍 Google auth URL: {}", auth_url);

    // 4) Open system browser using tauri_plugin_opener (finally using it!)
   window
    .opener()
    .open_url(auth_url.as_str(), None::<&str>)
    .map_err(|e| format!("Failed to open browser: {}", e))?;
println!("🚀 Opened system browser for Google login");

    // 5) Wait for Google redirect on loopback
    let request = server
        .incoming_requests()
        .next()
        .ok_or_else(|| "No OAuth callback received from Google".to_string())?;

    let full_url = format!("{}{}", redirect_uri, request.url());
    let parsed = Url::parse(&full_url).map_err(|e| format!("Invalid callback URL: {}", e))?;

    let code = parsed
        .query_pairs()
        .find(|(k, _)| k == "code")
        .map(|(_, v)| v.to_string())
        .ok_or_else(|| "Missing 'code' in Google callback".to_string())?;

    let state = parsed
        .query_pairs()
        .find(|(k, _)| k == "state")
        .map(|(_, v)| v.to_string())
        .ok_or_else(|| "Missing 'state' in Google callback".to_string())?;

    println!("✅ Received OAuth code + state from Google");

    // Respond to browser
    let _ = request.respond(Response::from_string(
        "Login successful. You can close this window and return to ClipTray.",
    ));
    ...
    let auth = oauth
        ...
        .await
        .map_err(|e| {
            let msg = format!("Failed to exchange code for tokens: {}", e);
            eprintln!("❌ {}", msg);
            msg
        })?;

    // 7) Fetch Google profile
    let user_info = oauth.get_user_info(&auth.access_token).await.map_err(|e| {
        let msg = format!("Failed to fetch Google user info: {}", e);
        eprintln!("❌ {}", msg);
        msg
    })?;

    println!(
        "✅ Google user: email={}, name={}, sub={}",
        user_info.email, user_info.name, user_info.sub
    );

    // 8) Map Google user -> your UsersRepository + session

    // Use a namespaced UID to avoid collisions with Firebase
    let google_uid = format!("google:{}", user_info.sub);
    let google_email = user_info.email.clone();
    let google_name = user_info.name.clone();

    // Check if user exists
    if let Some(existing_user) = UsersRepository::get_by_firebase_uid(&pool, &google_uid)
        .await
        .map_err(|e| format!("DB error while getting user: {}", e))?
    {
        println!("🟢 Returning existing Google user: {}", existing_user.email);

        let real_org_id = existing_user
            .organization_id
            .clone()
    ...
    println!(
        "📝 Creating new Google user: email={}, org_id={}",
        new_user.email, new_org_id
    );

    let created = UsersRepository::create_user(&pool, &new_user)
        .await
        .map_err(|e| {
            let msg = format!("Failed to create Google user: {}", e);
            eprintln!("❌ {}", msg);
            msg
        })?;
    ...
    println!("✅ Google user created & session set");
...
pub async fn validate_session(
    ...
    // Get DB pool if it exists
    let pool_state = match app.try_state::<sqlx::PgPool>() {
        Some(p) => p,
        None => {
            println!("⚠️ validate_session: DB pool not ready or not managed yet");
            // Returning Ok(None) is fine – frontend will treat as "not logged in"
            return Ok(None);
        }
    };

    let pool: &sqlx::PgPool = pool_state.inner();

    // Get current user info from in-memory session
...
pub async fn restore_session(
    ...
    let pool_state = match app.try_state::<sqlx::PgPool>() {
        Some(p) => p,
        None => {
            println!("⚠️ restore_session: DB pool not ready or not managed yet");
            return Ok(None);
        }
    };
    ...
    // 2) Fetch user by organization_id
    match UsersRepository::get_by_organization_id(pool, &organization_id).await {
        Ok(Some(user)) => {
            // organization_id is Option<String> in DB model
            let org_id = user
                .organization_id
                .clone()
                .unwrap_or_else(|| "default_org".to_string());

            // 3) Set in-memory session
            //    user_id = firebase_uid, org_id = real org id
            crate::session::set_current_user(
                user.firebase_uid.clone(),
                org_id.clone(),
                user.email.clone(),
        ...
        Ok(None) => {
            println!("❌ No user found for stored organization_id, clearing session");
            crate::session::clear_current_user();
            Ok(None)
        }
        Err(e) => {
            println!("❌ Error restoring session: {}", e);
...
pub async fn get_current_user(
    ...
    if let Some(session) = crate::session::get_current_session() {
        ...
        // Fetch full user from DB using firebase_uid (stored as user_id)
        let db_user = UsersRepository::get_by_firebase_uid(&pool, &session.user_id)
            .await
            .map_err(|e| format!("DB error fetching current user: {}", e))?;

        if let Some(u) = db_user {
            println!("✅ get_current_user: returning DB user {}", u.email);
...
pub async fn update_retain_tags_setting(
    ...
    // Use the user's numeric ID to update retain_tags
    let updated_user = UsersRepository::update_retain_tags(&pool, user.id, retain_tags)
        .await
        .map_err(|e| format!("Failed to update retain_tags: {}", e))?;

    println!(
        "✅ retain_tags updated successfully for user {} -> {}",
        updated_user.email, retain_tags
    );
...
pub async fn purge_untagged_entries(
    ...
    ClipboardRepository::delete_untagged_entries(&pool, &organization_id)
        .await
        .map_err(|e| e.to_string())
}
